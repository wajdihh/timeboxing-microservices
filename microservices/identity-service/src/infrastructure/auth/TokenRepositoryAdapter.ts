...
import { BaseInfraError, ID, ResultValue } from '@timeboxing/shared';
...
export class TokenRepositoryAdapter implements TokenRepository {
  ...
  async generateAccessToken(userId: string, email: string): Promise<ResultValue<string>> {
    ...
    try {
      ...
      return ResultValue.error(new BaseInfraError('Error generating access token', { detail: (error as Error).message }, undefined, 500));
